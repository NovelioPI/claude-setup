...
def main() -> int:
    ...
    verify_enabled = "VERIFY_ON_STOP=1" in raw or os.environ.get("CLAE_VERIFY_ON_STOP") == "1"
    if not verify_enabled:
        return 0

    code, out = run(["git", "diff", "--check"])
    if code != 0:
        sys.stderr.write("CLAE stop verification failed: `git diff --check`.\n" + out + "\n")
        return 2
    ...
    if verification.exists():
        ...
        for command in commands[:5]:
            ...
            if proc.returncode != 0:
                ...
                sys.stderr.write(f"CLAE stop verification failed: {command}\n{detail}\n")
                return 2
